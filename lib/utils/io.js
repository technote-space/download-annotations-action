...
const createFile = (workspace, filename, resultFilename, annotations) => {
    ...
    core_1.setOutput('ANNOTATIONS_NUMBER', annotationsArray.length);
    core_1.setOutput('ANNOTATION_MESSAGES', annotationsArray.map(annotation => annotation.message).join('\n'));
};
